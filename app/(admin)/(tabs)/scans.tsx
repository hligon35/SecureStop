...
const SCANS_WIDE_BREAKPOINT = 1024;
...
export default function AdminScansScreen() {
  ...
  const wideLayout = width >= SCANS_WIDE_BREAKPOINT;
  const filterModalWidth = Math.min(Math.max(width - 32, 0), 420);
  ...
  const filterActive =
    year !== null || month !== null || day !== null || timeOfDay !== "all";
  ...
  return (
    <View style={{ flex: 1, flexDirection: wideLayout ? "row" : "column" }}>
      <View style={{ flex: 1 }}>
        <View
          style={{
            ...
            >
              ...
              >
                  ...
                  >
                    ...
                    {monthOptions.map((m) => (
                      <Chip
                        ...
                        onPress={() => setFilters({ month: m.value })}
                      >
                        {m.label}
                      </Chip>
                    ))}
                ...
                <View style={{ gap: 8 }}>
                  ...
                  >
                    <Chip
                      selected={timeOfDay === "all"}
                      onPress={() => setFilters({ timeOfDay: "all" })}
                    >
                      All
                    </Chip>
                    <Chip
                      selected={timeOfDay === "morning"}
                      onPress={() => setFilters({ timeOfDay: "morning" })}
                    >
                      6:00–9:59
                    </Chip>
                    <Chip
                      selected={timeOfDay === "midday"}
                      onPress={() => setFilters({ timeOfDay: "midday" })}
                    >
                      10:00–13:59
                    </Chip>
                    <Chip
                      selected={timeOfDay === "afternoon"}
                      onPress={() => setFilters({ timeOfDay: "afternoon" })}
                    >
                      14:00–17:59
                    </Chip>
                    <Chip
                      selected={timeOfDay === "evening"}
                      onPress={() => setFilters({ timeOfDay: "evening" })}
                    >
                      18:00–21:59
                    </Chip>
                    <Chip
                      selected={timeOfDay === "overnight"}
                      onPress={() => setFilters({ timeOfDay: "overnight" })}
                    >
                      22:00–5:59
                    </Chip>
                  </View>
        ...
        <ScrollView
          contentContainerStyle={{
            ...
            paddingBottom: wideLayout
              ? 16
              : 16 + VEHICLE_CAROUSEL_HEIGHT + VEHICLE_CAROUSEL_BOTTOM_OFFSET,
          }}
        >
            ...
            <View style={{ paddingHorizontal: 4, paddingVertical: 12 }}>
              <Text style={{ color: theme.colors.onSurfaceVariant }}>
                {filterActive
                  ...
                  : "No scans recorded on this trip."}
              </Text>
            </View>
          ) : (
      ...
      {wideLayout ? (
        ...
        >
          ...
          <Text
            ...
            style={{ color: theme.colors.onSurfaceVariant }}
          >
            Choose a bus to filter and inspect trip scans.
          </Text>
          <ScrollView
